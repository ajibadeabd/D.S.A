...
function checkMagazine(magazine, note) {
	...
	let hasMag = magazine.split(" ").reduce((store,eachMag)=>{
		store[eachMag]=++store[eachMag]||1
		return store
	},{})
	console.log(hasMag )
	let number = 0
	let noteArray =note.split(" ")
	for(const eachNote of noteArray  ){
		if(hasMag[eachNote]){
			--hasMag[eachNote]
		number++
		}
	}
	console.log(number===noteArray.length?"Yes":'No')
...
// class MyClass {
// 	constructor() {
// 	  // Set the file path or module name as a property of the class
